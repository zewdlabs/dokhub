...
development-start-detached:
	...
	@UID=`id -u` GID=`id -g` docker-compose -f infra/compose.yaml -f infra/compose.development.yaml up --build -d

## HELP: development-start    : Start development platform (locally)
development-start:
	$(call environments-load,development)
	@UID=`id -u` GID=`id -g` docker-compose -f infra/compose.yaml -f infra/compose.development.yaml up --build

## HELP: development-stop     : Stop development platform (locally)
development-stop:
	$(call environments-load,development)
	@UID=`id -u` GID=`id -g` docker-compose -f infra/compose.yaml -f infra/compose.development.yaml down
...
production-deploy:
	...
	@docker-compose -f infra/compose.yaml -f infra/compose.production.yaml up --build --remove-orphans -d
...
production-logs-live:
	...
	@docker-compose -f infra/compose.yaml -f infra/compose.production.yaml logs --since 10s --follow

## HELP: production-logs      : Logs dump from production
production-logs:
	$(call environments-load,production)
	${INFO} "🕵️  Showing all logs"
	@docker-compose -f infra/compose.yaml -f infra/compose.production.yaml logs --timestamps
...
staging-deploy:
	...
	${INFO} "🚀 Deploying to ${DOCKER_CONTEXT}"
	@docker-compose -f infra/compose.yaml -f infra/compose.staging.yaml up --build --remove-orphans -d
	${INFO} "✅ Successfully deployed!"
...
staging-logs-live:
	$(call environments-load,staging)
	${INFO} "🕵️  Showing realtime logs"
	@docker-compose -f infra/compose.yaml -f infra/compose.staging.yaml logs --since 10s --follow

## HELP: staging-logs         : Logs dump from staging
staging-logs:
	$(call environments-load,staging)
	${INFO} "🕵️  Showing all logs"
	@docker-compose -f infra/compose.yaml -f infra/compose.staging.yaml logs --timestamps
